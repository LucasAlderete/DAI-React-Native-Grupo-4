  ...
  useEffect(() => { cargarClases(); }, []);
   ...
   const cargarClases = async () => {
      try {
        ...
        Alert.alert('Error', error.response?.data?.message || 'No se pudieron cargar las clases');
    ...
    return (
      <View style={styles.claseCard}>
        ...
        <View style={styles.cardBody}>
          ...
          <Text style={styles.direccionText}>{sede.direccion || 'Dirección no disponible'}</Text>
          <Text style={styles.fechaText}>🗓️ {formatearFecha(item.fechaInicio)}</Text>
        ...
        <View style={styles.cardFooter}>
          <View>
            <Text style={styles.lugaresText}>
              {hayLugar
                ? `${lugaresDisponibles} lugar${lugaresDisponibles !== 1 ? 'es' : ''} disponible${
                    lugaresDisponibles !== 1 ? 's' : ''
                  }`
                : 'Sin lugares disponibles'}
            </Text>
          </View>
          <TouchableOpacity
            style={[styles.reservarButton, !hayLugar && styles.reservarButtonDisabled]}
...
const styles = StyleSheet.create({
  ...
  searchInput: { backgroundColor: '#f5f5f5', borderRadius: 10, paddingHorizontal: 15, paddingVertical: 12, fontSize: 16, color: '#222' },
  loadingContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#f5f5f5' },
  loadingText: { marginTop: 10, fontSize: 16, color: '#666' },
  listContainer: { padding: 15 },
  emptyListContainer: { flex: 1 },
  claseCard: { backgroundColor: '#fff', borderRadius: 12, padding: 16, marginBottom: 12, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4, elevation: 3 },
  cardHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12 },
  headerLeft: { flex: 1 },
  disciplinaText: { fontSize: 18, fontWeight: 'bold', color: '#222', marginBottom: 4 },
  instructorText: { fontSize: 14, color: '#666' },
  capacidadBadge: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: 12, marginLeft: 10 },
  capacidadText: { color: '#fff', fontSize: 12, fontWeight: '600' },
  cardBody: { marginBottom: 12 },
  sedeText: { fontSize: 15, color: '#444', marginBottom: 4, fontWeight: '600' },
  direccionText: { fontSize: 13, color: '#888', marginBottom: 8, marginLeft: 20 },
  fechaText: { fontSize: 14, color: '#666', marginBottom: 6 },
  duracionText: { fontSize: 14, color: '#666', marginBottom: 6 },
  descripcionText: { fontSize: 13, color: '#888', fontStyle: 'italic', marginTop: 6 },
  ...
  lugaresText: { fontSize: 14, color: '#666' },
  reservarButton: { backgroundColor: '#007bff', paddingHorizontal: 20, paddingVertical: 10, borderRadius: 8 },
  reservarButtonDisabled: { backgroundColor: '#ccc' },
  reservarButtonText: { color: '#fff', fontSize: 14, fontWeight: '600' },
  emptyContainer: { flex: 1, justifyContent: 'center', alignItems: 'center', padding: 40 },
  emptyText: { fontSize: 80, marginBottom: 20 },
  emptyTitle: { fontSize: 22, fontWeight: 'bold', color: '#222', marginBottom: 10 },
  ...
  loadingOverlayText: { color: '#fff', fontSize: 16, marginTop: 10 },
});
